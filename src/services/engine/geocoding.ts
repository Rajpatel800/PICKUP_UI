  ...
  const data = await googleFetch<AutocompleteResponse>(
    `${PLACES_HOST}/places:autocomplete`,
    {
      ...
      headers: {
        ...
        'X-Goog-Api-Key': env.googleMapsApiKey as string,
      },
...
export const reverseGeocode = async (
  ...
  const params = new URLSearchParams({
    ...
    key: env.googleMapsApiKey as string,
  });
...
export const getCurrentPlace = async (signal?: AbortSignal): Promise<ResolvedPlace> => {
  ...
  // OS geocoder is not available in the Driver app natively yet, fallback to "Current location"
